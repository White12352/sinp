...
func newVMessClashParser(proxy map[string]any) (option.Outbound, error) {
	...
	if network, exists := proxy["network"].(string); exists {
		...
		case "ws":
			...
			options.Transport.WebsocketOptions, _ = convertWSTransport(proxy)
		case "http":
			options.Transport.Type = C.V2RayTransportTypeHTTP
			options.Transport.HTTPOptions, _ = convertHTTPTransport(proxy)
		case "h2":
			options.TLS.Enabled = true
			options.Transport.Type = C.V2RayTransportTypeHTTP
			options.Transport.HTTPOptions, _ = convertH2Transport(proxy)
		case "grpc":
			options.Transport.Type = C.V2RayTransportTypeGRPC
			options.Transport.GRPCOptions, _ = convertGRPCTransport(proxy)
		}
	}
