...
func newNativeURIParser(content string) ([]option.Outbound, error) {
	...
	for _, proxyRaw := range strings.Split(content, "\n") {
		...
		protocol := strings.ToLower(parsedArr[1])
		parsedProxy := TrimBlank(DecodeBase64Safe(parsedArr[2]))
		switch protocol {
		case "ss":
...
func newTuicNativeParser(content string) (option.Outbound, error) {
	...
	for _, addon := range strings.Split(result[5], "&") {
		...
		case "alpn":
			options.TLS.ALPN = strings.Split(value, ",")
		}
	}
...
func newVMessNativeParser(content string) (option.Outbound, error) {
	...
	for key, value := range proxy {
		...
		case "fp":
			options.TLS.Enabled = true
			options.TLS.UTLS.Enabled = true
			options.TLS.UTLS.Fingerprint = value
		case "net":
			switch value {
			case "ws":
				options.Transport.Type = C.V2RayTransportTypeWebsocket
				if host, exists := proxy["host"]; exists {
					for _, header := range strings.Split(fmt.Sprint("Host:", host), "\n") {
						...
						options.Transport.WebsocketOptions.Headers[key] = value
					}
				}
				if path, exists := proxy["path"]; exists {
					reg := regexp.MustCompile(`^/?(.*?)(?:\?ed=(\d*))?$`)
					result := reg.FindStringSubmatch(path)
					options.Transport.WebsocketOptions.Path = result[1]
					if result[2] != "" {
						options.Transport.WebsocketOptions.EarlyDataHeaderName = "Sec-WebSocket-Protocol"
						options.Transport.WebsocketOptions.MaxEarlyData = stringToUint32(result[2])
					}
				}
			case "h2":
				...
				options.TLS.Enabled = true
				if path, exists := proxy["path"]; exists {
					options.Transport.HTTPOptions.Path = path
				}
				if host, exists := proxy["host"]; exists {
					options.Transport.HTTPOptions.Host = []string{host}
				}
			case "tcp":
				if tType, exists := proxy["type"]; exists {
					if tType == "http" {
						...
						if method, exists := proxy["method"]; exists {
							options.Transport.HTTPOptions.Method = method
						}
						if path, exists := proxy["path"]; exists {
							options.Transport.HTTPOptions.Path = path
						}
						...
						if headers, exists := proxy["headers"]; exists {
							for _, header := range strings.Split(headers, "\n") {
								...
								options.Transport.HTTPOptions.Headers[key] = value
							}
						}
					}
				}
			case "grpc":
				options.Transport.Type = C.V2RayTransportTypeGRPC
				if host, exists := proxy["host"]; exists {
					options.Transport.GRPCOptions.ServiceName = host
				}
			}
		case "tfo", "tcp-fast-open", "tcp_fast_open":
			if value == "1" || value == "true" {
	...
	for key, value := range proxy {
		...
		case "type":
			...
			case "http":
				options.Transport.Type = C.V2RayTransportTypeHTTP
				if host, exists := proxy["host"]; exists {
					options.Transport.HTTPOptions.Host = strings.Split(host, ",")
				}
				if path, exists := proxy["path"]; exists {
					options.Transport.HTTPOptions.Path = path
				}
			case "grpc":
				options.Transport.Type = C.V2RayTransportTypeGRPC
				if serviceName, exists := proxy["serviceName"]; exists {
					options.Transport.GRPCOptions.ServiceName = serviceName
				}
			}
		case "security":
			if value == "tls" {
		...
		case "pbk":
			options.TLS.Enabled = true
			options.TLS.Reality.Enabled = true
			options.TLS.Reality.PublicKey = value
			if sid, exists := proxy["sid"]; exists {
				options.TLS.Reality.ShortID = sid
			}
		case "tfo", "tcp-fast-open", "tcp_fast_open":
			if value == "1" || value == "true" {
				options.TCPFastOpen = true
...
func newTrojanNativeParser(content string) (option.Outbound, error) {
	...
	for key, value := range proxy {
		...
		case "type":
			...
			case "grpc":
				options.Transport.Type = C.V2RayTransportTypeGRPC
				if serviceName, exists := proxy["grpc-service-name"]; exists {
					options.Transport.GRPCOptions.ServiceName = serviceName
				}
			}
		case "tfo", "tcp-fast-open", "tcp_fast_open":
...
func newHysteriaNativeParser(content string) (option.Outbound, error) {
	...
	for _, addon := range strings.Split(result[3], "&") {
		...
		case "alpn":
			options.TLS.ALPN = strings.Split(value, ",")
		case "ca":
			options.TLS.CertificatePath = value
		case "ca_str":
			options.TLS.Certificate = strings.Split(value, "\n")
		case "up":
			options.Up = value
		case "up_mbps":
